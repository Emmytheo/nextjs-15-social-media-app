...
interface PageProps {
    params: Promise<{ "org-name-id": string; programId: string }>;
}

export default async function Page({ params }: PageProps) {
    const { user } = await validateRequest();

    const { "org-name-id": orgNameOrId, programId } = await params;
        ...
        <div className="flex flex-1 flex-col gap-4 md:p-4 pt-0">
            <div className="flex items-center gap-4">
                ...
                <div className="flex-1 space-y-2">
                    <h2 className="text-xl md:text-2xl font-bold tracking-tight">{program.title}</h2>
