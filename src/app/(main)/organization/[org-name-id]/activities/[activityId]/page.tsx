...
interface PageProps {
    params: Promise<{ "org-name-id": string; activityId: string }>;
}

export default async function Page({ params }: PageProps) {
    const { user } = await validateRequest();
    // if (!user) redirect("/login"); // Allow public access

    const { "org-name-id": orgNameOrId, activityId } = await params;

    const activity = await prisma.organizationActivity.findUnique({
        where: { id: activityId },
        include: {
            organization: { // Include organization for admin check
                include: {
    ...
    if (!activity) redirect(`/organization/${orgNameOrId}/tabs/activities`);

    const isAdmin = user ? activity.organization.admins.some(a => a.userId === user.id) : false;

    return (
        <div className="flex flex-1 flex-col gap-4 md:p-4 pt-0">
            <div className="flex items-center gap-4">
                <Button variant="ghost" size="icon" asChild>
                    <Link href={`/organization/${orgNameOrId}/tabs/activities`}>
                        <ArrowLeft className="h-4 w-4" />
                    </Link>
                </Button>
                <div className="flex-1 space-y-2">
                    <h2 className="text-lg md:text-2xl font-bold tracking-tight">{activity.title}</h2>
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        ...
                        {activity.organizationProgram && (
                            <>
                                <span>•</span>
                                <Link href={`/organization/${orgNameOrId}/programs/${activity.organizationProgram.id}`} className="hover:underline">
                            ...
                            {activity.participants.length === 0 ? (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                    {activity.participants.map((p) => (
                                        <div key={p.user.id} className="flex items-center gap-2 p-2 border rounded-md">
